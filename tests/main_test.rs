...
use flare_rust_periphery_package::{
    constants, coston2, name_to_abi, name_to_address, names_to_addresses, Chain,
};
...
async fn get_abis_product_dot() {
    ...
    let contract = Contract::from_json(web3s.eth(), address, coston2::products::AddressBinder.abi)
        .expect("Failed to create a contract instance.");
    let result = contract.query(
        "cAddressToPAddress",
        address,
...
async fn get_abis_contract_name() {
    ...
    let contract = Contract::from_json(
        web3s.eth(),
        address,
        coston2::name_to_abi(String::from("AddressBinder")).expect("Could not find contract name"),
    )
    .expect("Failed to create a contract instance.");
    let result = contract.query(
        "cAddressToPAddress",
